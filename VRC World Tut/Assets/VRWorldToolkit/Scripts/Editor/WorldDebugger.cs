            ...
            try
            {
                ...
                if (sceneDescriptor.spawns != null && sceneDescriptor.spawns.Length > 0)
                {
                    ...
                    for (var i = 0; i < sceneDescriptor.spawns.Length; i++)
                    {
                        ...
                        if (!UnityEngine.Physics.Raycast(spawn.position + new Vector3(0, 0.01f, 0), Vector3.down, out RaycastHit hit, Mathf.Infinity, ~0, QueryTriggerInteraction.Ignore))
                        {
                            if (UnityEngine.Physics.Raycast(spawn.position + new Vector3(0, 0.01f, 0), Vector3.down, out hit, Mathf.Infinity))
                            {
